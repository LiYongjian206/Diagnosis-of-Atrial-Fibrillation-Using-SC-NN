...
Con_Matr = []
# 数据导入
data = np.load('D:\python_PAF\datamatrix.npy')
label = np.load('D:\python_PAF\labelmatrix.npy')
data = np.array(data)
...
model.summary()
plot_model(model, to_file='D:/python_PAF/model_matrix.png')#打印模型图

model.compile(loss='binary_crossentropy',
              optimizer='Adam', metrics=['accuracy'])
filepath = "D:/python_PAF/model_matrix.hdf5"  # 保存模型的路径
