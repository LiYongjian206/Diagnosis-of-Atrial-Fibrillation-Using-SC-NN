...
from keras.layers import Dense,  Flatten, Conv2D, MaxPooling2D, BatchNormalization, LeakyReLU, \
    GlobalAveragePooling2D, Activation, GlobalMaxPooling2D, \
     Concatenate, Add, Lambda, Dropout
...
data_test = np.load('D:\pythonProject\datamatrix.npy')
label_test = np.load('D:\pythonProject\labelmatrix.npy')
data = np.array(data_test)#125*10
X_train, X_test,y_train, y_test = train_test_split(data, label_test, test_size=0.2, random_state=3)
...
def models(input_shape):
    ...
    out = Model(inputs=[input_shape], outputs=[out], name="MyNet")
    return out
...
model.summary()
plot_model(model, to_file='D:/pythonProject/my_model.png')#打印模型图

model.compile(loss='binary_crossentropy',
              optimizer='Adam', metrics=['accuracy'])
filepath = "D:/pythonProject/my_model.hdf5"  # 保存模型的路径
...
train_history = model.fit(x=X_train,
                          ...
                          epochs=300, batch_size=128, verbose=2)
